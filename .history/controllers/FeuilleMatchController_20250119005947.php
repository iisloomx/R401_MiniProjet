                    ...
                    try {
                        foreach ($evaluations as $id => $evaluation) {
                            if ($evaluation < 1 || $evaluation > 5) {
                                echo "<script>alert('Les évaluations doivent être entre 1 et 5.'); window.location.href='FeuilleMatchController.php?action=evaluer&id_match=$id_match';</script>";
                                exit;
                            }
        ...
        case 'valider_modification':
            ...
            case 'supprimer':
                ...
                case 'valider_suppression':
                    ...
                    if (!$id_match) {
                        echo "<script>alert('ID du match non spécifié.'); window.location.href='FeuilleMatchController.php?action=afficher';</script>";
                        exit;
                    }
                    ...
                    if (empty($joueursASupprimer)) {
                        echo "<script>alert('Aucun joueur sélectionné pour suppression.'); window.location.href='FeuilleMatchController.php?action=supprimer&id_match={$id_match}';</script>";
                        exit;
                    }
                    ...
                    try {
                        ...
                        echo "<script>alert('Les joueurs sélectionnés ont été supprimés avec succès.'); window.location.href='FeuilleMatchController.php?action=afficher&id_match={$id_match}';</script>";
                        exit;
                    } catch (Exception $e) {
                        echo "<script>alert('Erreur lors de la suppression : {$e->getMessage()}'); window.location.href='FeuilleMatchController.php?action=supprimer&id_match={$id_match}';</script>";
                        exit;
                    }
