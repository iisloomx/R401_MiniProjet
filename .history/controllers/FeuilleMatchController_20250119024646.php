                ...
                case 'valider_evaluation':
                    ...
                    try {
                        foreach ($evaluations as $id => $evaluation) {
                            // Récupérer `numero_licence` et `id_match` à partir de la table `participer`
                            $query = "SELECT numero_licence FROM participer WHERE id = :id AND id_match = :id_match";
                            ...
                            if (!$row || empty($row['numero_licence'])) {
                                echo "<script>alert('Joueur non trouvé pour l\'id {$id} et match {$id_match}.'); window.location.href='FeuilleMatchController.php?action=evaluer&id_match={$id_match}';</script>";
                                exit;
                            }
                            ...
                            $participerModel->mettreAJourEvaluation([
