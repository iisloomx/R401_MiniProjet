        ...
        case 'ajouter':
            ...
            case 'evaluer':
                ...
                case 'valider_evaluation':
                    ...
                    if (!$id_match) {
                        echo "<script>alert('ID du match non spécifié.');</script>";
                        echo "<pre>";
                        echo "ID du match manquant.";
                        echo "</pre>";
                        exit;
                    }
                    ...
                    // Ajouter un var_dump pour vérifier les données POST
                    echo "<pre>";
                    echo "Données reçues dans POST : \n";
                    var_dump($_POST);
                    echo "</pre>";
                    ...
                    if (empty($evaluations)) {
                        ...
                        echo "</pre>";
                        echo "<script>alert('Aucune évaluation soumise.');</script>";
                        exit;
                    }
                    ...
                    try {
                        foreach ($evaluations as $numero_licence => $evaluation) {
                            ...
                            if (!is_numeric($evaluation) || $evaluation < 1 || $evaluation > 5) {
                                echo "<pre>";
                                echo "Évaluation invalide détectée : \n";
                                var_dump([
                                    'numero_licence' => $numero_licence,
                                    'evaluation' => $evaluation,
                                ]);
                                echo "</pre>";
                                echo "<script>alert('Les évaluations doivent être entre 1 et 5.');</script>";
                                exit;
                            }
                
                            // Debug avant la mise à jour
                            echo "<pre>";
                            echo "Mise à jour prévue pour : \n";
                            var_dump([
                                'numero_licence' => $numero_licence,
                                'id_match' => $id_match,
                                'evaluation' => $evaluation,
                            ...
                            $participerModel->mettreAJourEvaluation($numero_licence, $id_match, $evaluation);
                        }
                
                        // Si tout est validé, message de succès
                        echo "<pre>";
                        echo "Toutes les évaluations ont été enregistrées avec succès.";
                        echo "</pre>";
                        exit;
                    } catch (Exception $e) {
                        // Gestion des erreurs avec var_dump pour débogage
                        echo "<pre>";
                        echo "Erreur capturée lors de la mise à jour : \n";
