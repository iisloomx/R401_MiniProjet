...
switch ($action) {
    ...
    case 'modifier':
        // 1. Vérifier si un id_match est passé en GET
        ...
        // 2. Traitement si POST (formulaire de modification soumis)
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $data = [
                'id_match'           => $id_match, // on n'oublie pas l'ID pour le WHERE
                'equipe1'            => $_POST['equipe1'],
                'equipe2'            => $_POST['equipe2'],
                'score_equipe1'      => $_POST['score_equipe1'],
                'score_equipe2'      => $_POST['score_equipe2'],
        ...
        // 3. Si on n'est pas en POST, on récupère les infos du match à modifier
        $match = $gameMatch->obtenirMatch($id_match);
        // 4. On affiche le formulaire de modification pré-rempli
