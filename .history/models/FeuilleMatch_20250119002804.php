...
public function supprimerJoueur($id) {
    // Étape 1: Récupérer l'id_match correspondant avant suppression
    ...
    if (!$id_match) {
        return false; // Aucun match associé trouvé, on arrête la fonction
    }

    // Étape 2: Supprimer le joueur
    ...
    if (!$result) {
        return false; // Si la suppression échoue, on arrête ici
    }

    // Étape 3: Mettre à jour l'état de la feuille de match
    $queryUpdateEtat = "UPDATE match_ SET etat_feuille = 'Non validé' WHERE id_match = :id_match";
    $stmtUpdateEtat = $this->conn->prepare($queryUpdateEtat);
    $stmtUpdateEtat->bindParam(':id_match', $id_match);
    $updateResult = $stmtUpdateEtat->execute();

    // Retourner vrai seulement si les deux opérations ont réussi
    return $updateResult;
}
